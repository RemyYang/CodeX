        ...
        for i in range(count):
            ...
            for j in range(batch_size):
                if j == 0:
                  ...
                  t = sess.run(read_tensor_from_jpg_image_file_op,feed_dict={"fnamejpg:0": filenames[batch_size*i+j]})
...
#                      {input_operation.outputs[0]: t_batch})
        ...
        if file_num % batch_size !=0:
            ...
            for j in range(extra_num):
                if j == 0:
                    t_batch = sess.run(read_tensor_from_jpg_image_file_op,feed_dict={"fnamejpg:0": filenames[batch_size*count+j]})
                    t_batch = np.expand_dims(t_batch,axis = 0)
                else:
                    t = sess.run(read_tensor_from_jpg_image_file_op,feed_dict={"fnamejpg:0": filenames[batch_size*count+j]})
                    t = np.expand_dims(t,axis = 0)
                    t_batch = np.concatenate((t_batch,t), axis=0)
            print(t_batch.shape)
